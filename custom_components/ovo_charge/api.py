...
class OVOChargeApiClient:
  ...
  async def get_active_charge_session(self) -> Optional[Dict[str, Any]]:
    ...
    try:
      ...
      # Token might have expired, try one more time after refresh
      await self.refresh_id_token()
      if not self._id_token:
          raise OVOChargeAuthError("Unable to refresh id_token.")
      headers["Authorization"] = self._id_token
      ...
      # Other API errors, handled by the coordinator
      raise
    return None
  ...
  async def verify_magic_link(
        ...
        try:
            ...
            if not oob_code:
                raise OVOChargeAuthError(
                    "oobCode not found in the provided link."
                )
        except Exception as e:
            # Catch any parsing errors (e.g., malformed URL)
            raise OVOChargeAuthError(
                f"Invalid magic link format: {e}"
            ) from e
        ...
        try:
            async with session.post(
                url, headers=REQUEST_HEADERS, json=body
            ) as resp:
                resp.raise_for_status()
